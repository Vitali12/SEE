...
def calculate_stress(load, A):
    ...
    A_m2 = A * 1e-6      # перевод площади из мм² в м²
...
def convert_strain(strain, H):
    return strain / H  # преобразование деформации
...
# Функция для расчёта модуля Юнга
...
def plot_graph():
    ...
    try:
        ...
        data_dict = {
            "Оригинальные данные": pd.DataFrame({"strain_rel": strain_rel, "stress": stress}),
            "Аппроксимированные данные": pd.DataFrame({"approx_strain_rel": approx_strain, "approx_stress": approx_stress}),
            "Модуль Юнга": pd.DataFrame({"strain_rel": approx_strain[1:], "youngs_modulus": youngs_moduli}),
        }
...
tk.Label(root, text="Площадь поперечного сечения (мм²)").grid(row=2, column=0, padx=10, pady=5)
...
tk.Checkbutton(root, text="Показать оригинальную кривую", variable=show_original).grid(row=6, column=0, padx=10, pady=5)

show_youngs_modulus = tk.IntVar(value=1)
tk.Checkbutton(root, text="Показать модуль Юнга", variable=show_youngs_modulus).grid(row=6, column=1, padx=10, pady=5)
